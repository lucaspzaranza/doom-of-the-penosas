...
public class ObjectPool : MonoBehaviour
{
    ...
    void OnEnable()
    {
        PauseMenuEvents.OnBackToMainMenu += ReturnAllGameObjects;
    }
    ...
    public void ReturnAllGameObjects()
    {
        foreach (var kvp in objectPool)
        {
            Transform poolParent = transform.Find($"{kvp.Key} Pool");
            if (poolParent == null) continue;
            ...
            foreach (Transform child in poolParent)
            {
                ...
                if (obj.activeSelf)
                {
                    ...
                    float yScale = obj.transform.localScale.y;
                    obj.transform.localScale = new Vector2(Mathf.Abs(xScale), Mathf.Abs(yScale));

                    obj.SetActive(false);

                    // Só enfileira se ainda não estiver na fila
