...
public class PlayerController : ControllerUnit, IPlayerController
{
    // Events
    public event Action<byte, bool> OnCountdownActivation;
    public event Action<bool> OnPlayerPause;
    public event Action<byte> OnPlayerGameOver;
    public event Action<IReadOnlyList<Penosas>> OnPlayersExchanged;
    ...
    public IList<IPlayerData> PlayersData => _playersData.Cast<IPlayerData>().ToList();

    [SerializeField] private List<GameObject> _rideArmors;

    public Transform Transform => transform;
    public GameMode GameMode => TryToGetGameControllerFromParent().GameMode;
    public GameStatus GameStatus => TryToGetGameControllerFromParent().GameStatus;
    ...
    public void Setup(IReadOnlyList<Penosas> characters, IReadOnlyList<InputDevice> selectedDevices = null)
    {
        ...
        PlayerEvents.GetPlayerStartPosition = () => PlayerStartPosition;
    }

    private void PlayersEventSetup(IPlayerCharacter player)
    {
    ...
    public void AddPlayers()
    {
        ...
        PlayerEvents.GetFirstPlayerTransform = () =>
            PlayersData.FirstOrDefault().Player.Transform;
    }
    ...
    private void PlayerDataSetup(IPlayerData playerData, GameObject newPlayer)
    {
        var newPlayerScript = newPlayer.GetComponent<IPlayerCharacter>();
        ...
        var cameraSelector = FindFirstObjectByType<CinemachineCameraSelector>();
        newPlayerScript.SetCameraSelector(cameraSelector);
        ...
        CameraEvents.OnPlayerReadyToFollow?.Invoke(playerData.Player.Transform);
    }
    ...
    public void ExchangePlayers()
    {
        ...
        PlayerData dataAux = _playersData[0];
        _playersData[0] = _playersData[1];
        _playersData[1] = dataAux;
    ...
    public void HandleOnPlayerRideArmor(byte playerID, IRideArmor rideArmor, bool isEquipping)
    {
    ...
    public void InvokeOnPlayerPause(bool value)
    {
        OnPlayerPause?.Invoke(value);
    }

    public void AddOnCountdownActivationListener(Action<byte, bool> listener)
    {
        if (OnCountdownActivation == null)
            OnCountdownActivation += listener;
    }

    public void AddOnPlayerPauseListener(Action<bool> listener)
    {
        if (OnPlayerPause == null)
            OnPlayerPause += listener;
    }

    public void AddOnPlayerGameOverListener(Action<byte> listener)
    {
        if (OnPlayerGameOver == null)
            OnPlayerGameOver += listener;
    }

    public void AddOnPlayersExchangedListener(Action<IReadOnlyList<Penosas>> listener)
    {
        if (OnPlayersExchanged == null)
            OnPlayersExchanged += listener;
    }
}
